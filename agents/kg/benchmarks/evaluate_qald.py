...
def alias_sets(values: set[str], connector: Any = None) -> list[set[str]]:
    ...
    items: list[set[str]] = []
    # l'ordine di iterazione di un set dipende da PYTHONHASHSEED: senza sorted() la
    # stessa run darebbe punteggi diversi fra processi quando due risposte collidono
...
def _max_matching(gold_items: list[set[str]], our_items: list[set[str]]) -> int:
    ...
    def augment(gold_index: int, visited: set[int]) -> bool:
        for i, our_forms in enumerate(our_items):
            if i in visited or not (gold_items[gold_index] & our_forms):
                continue
            visited.add(i)
            if i not in assigned or augment(assigned[i], visited):
                assigned[i] = gold_index
                return True
        return False

    return sum(1 for gold_index in range(len(gold_items)) if augment(gold_index, set()))
...
def executed_gold(entries: list[dict], benchmark: Benchmark, provider: Any) -> dict[str, Any]:
    ...
    for i, entry in enumerate(pending, start=1):
        ...
        try:
            ...
            cache[key] = None
        if i % 25 == 0:
            print(f"  gold rigenerato: {i}/{len(pending)}")
...
def main() -> None:
    ...
    regenerated_gold = executed_gold(entries, benchmark, provider) if args.gold == "executed" else {}
    ...
    if len(usable) != len(entries):
        origin = "rigenerato eseguendo le query gold" if regenerated_gold else "registrato nel dataset"
        print(f"riferimento {origin}: {len(entries) - len(usable)} domande senza gold utilizzabile, escluse")
        entries = usable
    ...
    if not results:
        print("nessuna domanda valutabile: campione vuoto o nessun gold utilizzabile")
        return
    ...
    report = REPORT_DIR / f"{args.benchmark}_{time.strftime('%Y%m%d_%H%M%S')}.json"
    report.write_text(
        json.dumps(
            {"benchmark": benchmark.label, "target_kg": benchmark.target_kg, "gold": args.gold,
             "macro_f1": macro_f1, "results": results},
