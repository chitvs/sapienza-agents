"""
Test su dimensioni di complessita' MAI verificate nelle sessioni precedenti: proprieta'
...
confermare fix gia' fatti — a differenza degli altri file test_pipeline*.py, nessuna di
queste domande e' mai stata eseguita contro la pipeline reale: e' lecito che qualcuna
fallisca, e' proprio quello che vogliamo scoprire.
"""
import pytest
import requests
...
def test_switzerland_official_languages_list():
    """proprieta' multivalore: la Svizzera ha 4 lingue ufficiali, non 1 - verifica che il
...
def test_titanic_director_citizenship_capital():
    """catena a 3 hop: Titanic -> regista (James Cameron) -> cittadinanza (Canada) -> capitale (Ottawa)."""
    pipeline = KGPipeline()
    result = pipeline.run("What is the capital of the country of citizenship of the director of Titanic?")
    assert len(result.results) > 0
    assert any("Ottawa" in str(row) for row in result.results)
...
def test_bach_children_count():
    """aggregazione COUNT su un'entita' persona, nessuna verita' esatta assunta (i dati
    storici variano leggermente a seconda della fonte) - verifica solo che il sistema
    produca un conteggio plausibile, non zero/vuoto."""
    pipeline = KGPipeline()
...
def test_highest_mountain_eiffel_tower_country():
    """multi-hop attraverso un'entita' "ancora" insolita (un monumento, non un paese/persona
    citato direttamente): Torre Eiffel -> paese (Francia) -> montagna piu' alta (Monte Bianco)."""
    pipeline = KGPipeline()
...
def test_official_language_capital_paris():
    ...
    la cui capitale e' Parigi" -> lingua ufficiale (Francese)."""
    pipeline = KGPipeline()
...
def test_mars_moons_count():
    ...
    esattamente 2 lune: Phobos e Deimos) per avere un confronto con verita' nota."""
    pipeline = KGPipeline()
