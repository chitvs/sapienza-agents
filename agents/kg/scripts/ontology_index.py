...
def build_and_save(properties: list[dict], classes: list[dict], output_dir: Path) -> None:
    ...
    for name, items in (("properties", properties), ("classes", classes)):
        ...
        faiss.write_index(create_faiss_index(emb.astype(np.float32)), str(output_dir / f"{name}.faiss"))
        meta = [{"index": i, **item} for i, item in enumerate(items)]
        (output_dir / f"{name}_meta.json").write_text(
            json.dumps(meta, indent=2, ensure_ascii=False), encoding="utf-8"
        )
        logger.info("  %s.faiss: %d voci", name, len(items))
